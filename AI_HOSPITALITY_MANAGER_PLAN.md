# xFlow AI Hospitality Manager - piano di prodotto e implementazione
...
La demo non deve vendere un canvas. Deve vendere l'idea che un manager hospitality possa avere un copilota operativo che:

- capisce cosa sta succedendo nella struttura;
- raggruppa eventi, messaggi, tool call e task dentro casi operativi;
- decide cosa automatizzare e cosa portare all'umano;
...
### 2.3 Il vecchio prototipo WhatsApp aveva elementi utili

Dal file di specifiche WhatsApp precedente sono utili:

- layer di orchestrazione intelligente;
- trigger webhook, scheduled e manuali;
- agenti con tool;
- adapter/provider discovery;
- dashboard task;
- ruoli diversi tra manager, operatori e receptionist;
- correlation ID;
- audit;
- handoff;
- notifiche;
...
### 2.4 Cosa non riprendere dal vecchio prototipo

Non ricostruire:

- CRUD PMS nativo come core del prodotto;
- task, notifiche e run come mondi separati;
- single-agent config come centro dell'UX;
- generazione flow tramite keyword training;
- chat generica con agenti come interfaccia principale;
- KPI generici senza significato operativo;
- outcome binario post-esecuzione tipo `achieved: boolean` come meccanismo principale.

## 3. Nuova architettura di prodotto

Il prodotto deve avere due piani:

1. **Manager Cockpit**
   - primo schermo;
   - operativo;
   - orientato a casi, rischio, SLA e decisioni;
   - mostra cosa il manager deve sapere e fare.

2. **Automation Studio**
   - secondo piano;
   - tecnico-operativo;
   - mostra canvas, nodi, runtime, prompt, tool, fallback e success contract.
...
La top navigation deve diventare:
...
`Cockpit` deve essere la view attiva di default.

## 6. Manager Cockpit

### 6.1 Obiettivo

Il Cockpit deve rispondere a queste domande in pochi secondi:
...
1. briefing;
2. KPI;
3. lista casi scrollabile;
4. dettaglio caso;
5. CTA sticky o ben visibili.

Non deve esserci overflow orizzontale a 390 px.

### 6.4 KPI utili

Non usare metriche decorative. Esempi utili:

- arrivi oggi;
- casi attivi;
- casi a rischio SLA;
- interventi umani aperti;
- ospiti in attesa;
- automazione completata senza intervento;
- issue tecniche non bloccanti;
...
```js
[
  { label: "Arrivi oggi", value: "17", trend: "+4 dalle 15:00" },
  { label: "Casi attivi", value: "6", trend: "3 a rischio SLA" },
  { label: "Interventi umani", value: "2", trend: "1 critico" },
  { label: "Automazione", value: "84%", trend: "ultime 24h" }
]
```
