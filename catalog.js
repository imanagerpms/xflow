(() => {
  ...
  function createFlow(spec) {
    const ids = {
      ...
      task: `${spec.id}-imanager-task`,
      outcome: `${spec.id}-outcome`,
    };
    const procedure = spec.steps.map((step, index) => `${index + 1}. ${step}.`).join("\n");
    const messageTool = spec.messageTool || "guest.sendMessage";
    const taskDefinition = {
      title: `Completare manualmente: ${spec.name}`,
      objective: spec.objective,
      tasklist: "iManager",
      priority: spec.risk === "Alto" ? "alta" : "media",
      requiredContext: [
        "booking_id",
        "ota_reference",
        "guest_contact",
        "stay_dates",
        "unit_id",
        "errore_o_motivo_escalation",
      ],
      instructions: [
        "Raggiungere manualmente lo stesso obiettivo operativo del flusso.",
        "Usare solo i dati minimi necessari presenti nel task.",
        "Registrare esito, evidenza e riferimento nel PMS o nella timeline iManager.",
      ],
      privacy: "Non includere dati personali non necessari, note interne irrilevanti o log tecnici completi.",
    };
    const agentTools = [
      {
        function: spec.tool,
        label: spec.toolLabel,
        purpose: `Esegue l'azione principale per ${spec.objective}.`,
        critical: true,
      ...
      },
      {
        ...
        label: "Comunica esito e aggiorna timeline",
        purpose: "Invia solo l'esito verificato e registra la decisione operativa.",
        critical: false,
        failureRoute: ids.task,
      ...
      },
      {
        ...
        purpose: "Apre un task manuale per ogni escalation, fallback o failure definitiva.",
        critical: true,
    ...
    return {
      ...
      metrics: { sla: spec.sla, risk: spec.risk, agenti: "1 agente", automations: `${agentTools.length} tool` },
      simulationOrder: [ids.trigger, ids.agent, ids.tool, ids.message, ids.task, ids.outcome],
      edges: [
        { from: ids.trigger, to: ids.agent, outcome: "evento_validato" },
        { from: ids.agent, to: ids.tool, outcome: "azione_tool" },
        { from: ids.agent, to: ids.message, outcome: "comunicazione" },
        { from: ids.agent, to: ids.task, outcome: "fallback_o_escalation" },
        { from: ids.tool, to: ids.message, outcome: "esito_verificato" },
        { from: ids.message, to: ids.outcome, outcome: "chiusura" },
        { from: ids.task, to: ids.outcome, outcome: "completato_manualmente" },
      ],
      nodes: [
        {
          id: ids.trigger, type: "trigger", x: 42, y: 280, name: spec.name,
        ...
        {
          id: ids.agent, type: "agent", x: 350, y: 180, name: `Agente ${spec.role}`,
          description: `Responsabile unico del flusso: capisce l'evento, usa i tool disponibili e apre task iManager quando serve intervento umano.`,
          condition: "event.validation == 'pass'",
          businessGoal: spec.objective,
          capability: "manage_flow_with_tools_and_imanager_fallback",
          tools: agentTools,
          taskTemplates: [taskDefinition],
          policy: {
            owner: `Agente ${spec.role}`,
            checks: ["deduplica", "policy", "autorizzazione", "dati_minimi", "success_contract"],
            escalationRule: "Ogni fallback o escalation crea sempre un task iManager prima di fermare il flusso.",
            taskDataRule: "Il task contiene solo informazioni necessarie al completamento manuale dell'obiettivo.",
          },
          prompt: `Sei l'agente responsabile del flusso "${spec.name}" per una struttura hospitality italiana.\n\nObiettivo operativo: ${spec.objective}.\n\nContesto disponibile: ${spec.inputs}. Usa solo dati presenti nei sistemi collegati; se manca un dato decisivo, crea un task iManager invece di chiedere dati non necessari.\n\nIstruzioni operative:\n${procedure}\n\nTool a disposizione:\n${agentTools.map((tool) => `- ${tool.function}: ${tool.purpose}`).join("\n")}\n\nPolicy:\n- ${spec.guardrail}.\n- Non dichiarare mai completata un'azione finche il tool o il task iManager non restituisce un esito verificabile.\n- Ogni escalation, fallback, timeout o dato bloccante deve creare un task nella tasklist iManager.\n- Il task iManager deve contenere solo: obiettivo manuale, dati minimi necessari, scadenza, evidenze richieste e riferimento prenotazione.\n- Non inserire nel task log tecnici completi, dati personali non necessari o istruzioni che non aiutano l'operatore a completare l'obiettivo.\n\nFallback operativi:\n1. Se il tool principale fallisce, apri un task iManager per raggiungere manualmente "${spec.objective}".\n2. Se manca una policy o autorizzazione, apri un task iManager con la sola verifica richiesta.\n3. Se la comunicazione automatica non e' consegnata, apri un task iManager per consegnare l'esito su canale tracciabile.\n\nTono: calmo, concreto, professionale e non difensivo.`,
          guardrail: spec.guardrail,
          fallbackPlaybook: [
            "Creare task iManager quando un tool critico fallisce definitivamente.",
            "Creare task iManager quando serve autorizzazione o verifica manuale.",
            "Creare task iManager quando il dato mancante blocca il success contract.",
          ],
          params: {
            available_tools: agentTools.map((tool) => tool.function),
            imanager_task_policy: taskDefinition,
            policy_checks: ["deduplica", "autorizzazione", "privacy", "success_contract"],
          },
        },
        {
          id: ids.tool, type: "tool", x: 680, y: 98, name: spec.toolLabel,
          description: `Intervento dell'agente: esegue l'azione operativa principale tramite ${spec.tool}.`,
          condition: "agent.policy == 'pass'", tool: spec.tool,
          agentIntervention: true,
          agentId: ids.agent,
          agentLabel: `Agente ${spec.role}`,
          params: { booking_id: "{{booking.id}}", ...spec.params },
          guardrail: "Verifica la risposta del tool e annota l'esito; timeout o errore aprono un task iManager.",
        },
        {
          id: ids.message, type: "message", x: 680, y: 370,
          name: spec.messageTool === "team.notify" ? "Notifica team e registra" : "Aggiorna ospite e PMS",
          description: "Intervento dell'agente: comunica l'esito sul canale corretto e chiude il ciclo con una nota operativa.",
          condition: "tool.status in ['succeeded','needs_attention']", tool: messageTool,
          agentIntervention: true,
          agentId: ids.agent,
          agentLabel: `Agente ${spec.role}`,
          params: { template: spec.template, language: "{{guest.language}}", add_pms_timeline_note: true },
        ...
        {
          id: ids.task, type: "human_task", x: 970, y: 235, name: "Task iManager fallback",
          description: "Intervento dell'agente: apre una tasklist manuale con il solo obiettivo da raggiungere e il contesto minimo.",
          ...
          taskDefinition,
          params: taskDefinition,
          guardrail: "Il task non contiene log tecnici completi o dati personali non necessari.",
          outcomes: { completed_manually: ids.outcome },
        },
        {
          id: ids.outcome, type: "outcome", x: 1248, y: 260, name: "Obiettivo completato",
          description: `Success contract: ${spec.objective}.`,
          condition: "tool.status == 'succeeded' || task.status == 'completed'",
          businessGoal: spec.objective,
          params: { success_contract: spec.objective },
          guardrail: "Chiude solo con evidenza verificabile da tool o task iManager.",
        },
      ],
    };
  }
